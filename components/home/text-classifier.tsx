               ...
               <Card.Body css={{ py: '$6', px: '$8' }}>
                  ...
                  >
                     <Text css={{ color: '$accents5', fontSize: '11px' }}>
                        <em>Catatan: Menggunakan model {method === 'tfidf' ? 'TF-IDF + SVM' : method === 'fasttext' ? 'FastText + SVM' : method.includes('dt') ? method.replace('dt-', '').toUpperCase() + ' + Decision Tree' : method.includes('nb') ? method.replace('nb-', '').toUpperCase() + ' + Naive Bayes' : method} yang dilatih dengan ICAR Agriculture Dataset. Data dilayani via FastAPI.</em>
                     </Text>
                  </Box>
               </Card.Body>
            </Card>
